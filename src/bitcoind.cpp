...
static bool AppInit(NodeContext& node, int argc, char* argv[])
{
    ...
    // If Qt is used, parameters/cpuchain.conf are parsed in qt/cpuchain.cpp's main()
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see cpuchaind -h for a list of options.\n", argv[i])));
            }
        }
        ...
        // -server defaults to true for cpuchaind but not for the GUI so do this here
...
int main(int argc, char* argv[])
{
    ...
    // Connect cpuchaind signal handlers
    noui_connect();
