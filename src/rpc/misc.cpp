    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (interfaces::Ipc* ipc = Assert(EnsureAnyNodeContext(request.context).init)->ipc()) {
                // Spawn a new cpuchain-node process and call makeEcho to get a
                // client pointer to a interfaces::Echo instance running in
                // that process. This is just for testing. A slightly more
                // realistic test spawning a different executable instead of
                // the same executable would add a new cpuchain-echo executable,
                // and spawn cpuchain-echo below instead of cpuchain-node. But
                // using cpuchain-node avoids the need to build and install a
                // new executable just for this one test.
                auto init = ipc->spawnProcess("cpuchain-node");
                echo = init->makeEcho();
                ipc->addCleanup(*echo, [init = init.release()] { delete init; });
            } else {
                // IPC support is not available because this is a cpuchaind
                // process not a cpuchaind-node process, so just create a local
